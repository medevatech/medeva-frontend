...
const VitalSigns = ({ match }) => {
  ...
  let urlPasien = `https://medeva-backend-production.up.railway.app/api/v1/pasien`;

  const onLoadPatient = async () => {
    try {
      const response = await axios.get(urlPasien);
    ...
    <>
      <Row>
        ...
        <Colxx sm="12" md="12" xl="8" className="mb-4">
          <Card className="mb-8">
            <CardBody>
              <CardTitle>Form Registrasi Pra-Konsultasi</CardTitle>
              <Form>
                <FormGroup row>
                  <Colxx sm={6}>
                    <FormGroup>
                      <Label for="tanggalRekam">Tanggal / Waktu</Label>
                      <DatePicker
